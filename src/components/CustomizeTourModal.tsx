...
import { Sparkles, X, Check, Calendar, Users, MapPin, Hotel, Car, MessageSquare, Phone, Mail, User, ShieldCheck, ArrowRight, HeartHandshake } from "lucide-react";
  ...
  return (
    <>
      ...
      <button
        ...
        className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500 via-amber-500 to-orange-600 text-white px-6 py-3 rounded-full font-bold hover:from-orange-600 hover:to-amber-600 transition-all shadow-lg shadow-orange-500/30 transform hover:-translate-y-0.5 border border-orange-400/30 group"
      >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
          <div 
            className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden my-8 animate-in zoom-in-95 duration-300 text-slate-800"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div className="relative bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-6 sm:p-8 text-white">
              <button
                onClick={handleClose}
            ...
            {step === "form" ? (
              <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
                    ...
                    {DESTINATIONS_LIST.map((dest) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center justify-between p-2.5 rounded-xl border text-left transition-all text-xs sm:text-sm font-semibold ${
                            isSelected
                ...
                <div className="pt-2 flex items-center justify-end gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white px-8 py-3.5 rounded-full font-bold text-sm sm:text-base hover:from-orange-600 hover:to-amber-600 transition-all shadow-lg shadow-orange-500/30 disabled:opacity-70 disabled:cursor-not-allowed transform hover:-translate-y-0.5"
                  >
                    {isSubmitting ? (
                      <>
                      ...
                      <>
                        ...
                        <span>Get Free Custom Quote on WhatsApp</span>
                      </>
                    )}
                  </button>
                </div>
              ...
              <div className="p-8 sm:p-12 text-center space-y-6">
                ...
                <div className="pt-2">
                  <button
                    onClick={handleClose}
                    className="bg-slate-900 text-white px-8 py-3.5 rounded-full font-bold text-sm hover:bg-slate-800 transition-all shadow-md"
                  >
