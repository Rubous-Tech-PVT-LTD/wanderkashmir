...
export default function GuideClient({ bookings = [], vendorProfileId, initialGuideProfile }: { bookings?: any[], vendorProfileId?: string, initialGuideProfile?: any }) {
      ...
      <div className="flex gap-6 border-b border-slate-200 mb-8 overflow-x-auto whitespace-nowrap">
        {["overview", "profile", "bookings", "promo"].map((tab) => (
          <button
            ...
            className={`pb-4 text-sm font-semibold capitalize transition-colors relative ${activeTab === tab ? "text-orange-500" : "text-slate-500 hover:text-slate-800"}`}
          >
            {tab}
      ...
      {activeTab === "profile" && (
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className="p-6">
              {isProfileLocked && (
                <div className="bg-orange-500 border border-orange-500 rounded-xl p-5 mb-8 flex items-start gap-3">
                  <Lock className="w-5 h-5 text-orange-500 mt-0.5 shrink-0" />
                  <div>
                    <h3 className="font-bold text-orange-500 text-sm">Your limit reached</h3>
                    <p className="text-sm text-orange-500 mt-1">Your profile is complete and has been saved! On the Free plan, profiles cannot be edited after completion from this tab. Please upgrade to Growth Pro to unlock unlimited profile edits and priority ranking.</p>
                    <button type="button" onClick={() => { setActiveTab("financials"); setIsEditMode(false); }} className="mt-3 text-xs font-bold text-white bg-orange-500 px-4 py-2 rounded-lg hover:bg-orange-500 transition-colors">View Upgrade Plans</button>
                  </div>
                </div>
              )}
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl space-y-6">
                <div>
                  ...
                  <textarea disabled={isProfileLocked} rows={5} {...register("bio")} className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.bio ? 'border-orange-500' : 'border-slate-200'} ${isProfileLocked ? 'bg-slate-50 text-slate-500 cursor-not-allowed' : ''}`} placeholder="Tell travelers about your experience, your connection to Kashmir, and what makes your tours special..." />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input disabled={isProfileLocked} type="text" {...register("location")} className={`w-full border rounded-lg pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.location ? 'border-orange-500' : 'border-slate-200'} ${isProfileLocked ? 'bg-slate-50 text-slate-500 cursor-not-allowed' : ''}`} placeholder="e.g. Srinagar, Gulmarg" />
                    </div>
                  ...
                  <div>
                    ...
                    <input disabled={isProfileLocked} type="text" {...register("languages")} className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.languages ? 'border-orange-500' : 'border-slate-200'} ${isProfileLocked ? 'bg-slate-50 text-slate-500 cursor-not-allowed' : ''}`} placeholder="e.g. English, Hindi, Kashmiri" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input disabled={isProfileLocked} type="number" {...register("experienceYears", { valueAsNumber: true })} className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.experienceYears ? 'border-orange-500' : 'border-slate-200'} ${isProfileLocked ? 'bg-slate-50 text-slate-500 cursor-not-allowed' : ''}`} placeholder="5" />
                ...
                <div>
                  ...
                  <input disabled={isProfileLocked} type="text" {...register("specializations")} className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.specializations ? 'border-orange-500' : 'border-slate-200'} ${isProfileLocked ? 'bg-slate-50 text-slate-500 cursor-not-allowed' : ''}`} placeholder="e.g. History Walks, Trekking, Photography" />
                ...
                <div className={`p-6 rounded-xl border mt-6 ${hasInstantBooking ? 'bg-orange-500 border-orange-500' : 'bg-slate-50 border-slate-200 opacity-70'}`}>
      ...
      {activeTab === "bookings" && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-6 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            ...
            <button className="flex items-center gap-2 bg-orange-500 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-orange-500 transition-colors">
