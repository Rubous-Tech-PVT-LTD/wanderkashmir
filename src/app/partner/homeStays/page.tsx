...
export default async function HomestayDashboardPage() {
  ...
  try {
    ...
    console.error("Error fetching vendor homestay properties with roomTypes:", err);
    try {
      properties = await prisma.property.findMany({
        where: { vendorProfileId: session.vendorProfileId },
        orderBy: { createdAt: "desc" },
      ...
      console.error("Fallback homestay property query error:", fallbackErr);
      properties = [];
    }
  }
  ...
  return <HomestayClient vendorProfileId={session.vendorProfileId} properties={safeProperties} bookings={safeBookings} />;
}
