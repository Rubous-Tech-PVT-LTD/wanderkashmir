...
import { propertySchema, PropertyData } from "@/lib/validations";
...
// We extend PropertyData with local state needed for the UI simulation (rooms, instantBooking)
const hotelListingSchema = propertySchema.extend({
  totalRooms: z.number().min(1, "Must have at least 1 room").max(500),
  instantBooking: z.boolean().optional(),
});

type HotelListingFormValues = z.infer<typeof hotelListingSchema>;
...
export default function HotelClient({ vendorProfileId, properties = [], bookings = [] }: { vendorProfileId: string, properties?: any[], bookings?: any[] }) {
  ...
  const { totalRevenue, totalBookings, totalViews, chartData, growthRevenue, growthBookings, growthViews } = calculateDashboardMetrics(bookings, "HOTEL", timeRange);
  ...
  const demographicData = [
    { name: 'Delhi NCR', value: 45 },
    { name: 'Mumbai', value: 25 },
    { name: 'Gujarat', value: 15 },
    { name: 'Foreign', value: 5 },
    { name: 'Other', value: 10 },
  ];
  const COLORS = ['#0ea5e9', '#6366f1', '#f59e0b', '#10b981', '#cbd5e1'];
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<HotelListingFormValues>({
  ...
  const onSubmit = async (data: HotelListingFormValues) => {
    if (!isApproved) {
      toast.error("Error: Your profile is pending Admin approval. You cannot publish this listing yet.");
      return;
    }
    if (!coverPhoto) {
      toast.error("Please upload a Cover Photo for your listing.");
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        ...
        if (hasReachedLimit) {
          toast.error("You have reached the maximum limit of 1 property on the Free plan. Please upgrade to add more properties.", { duration: 5000 });
  ...
  const subscriptionTiers = [
    ...
    { name: "Growth Pro", price: "₹999/mo", description: "Get more visibility and bookings", features: ["Priority Listing", "20 Property Photos", "Advanced Analytics", "Onboarding Helpline (WhatsApp)", "Help Center & Tutorials (Hindi/English)"], isPopular: true },
    { name: "Pro", price: "₹2,499/mo", description: "For established hotels", features: ["All Growth Pro Features", "Featured Badge & Homepage Placement", "Instant Booking Option", "Up to 50 Property Photos", "Custom Promotional Offers"], isPopular: false },
    { name: "Enterprise", price: "₹4,999/mo", description: "For large hotel chains", features: ["All Pro Features", "API Access & Bulk Management", "Dedicated Account Manager", "Free Professional Photography", "Trilingual Support (Kashmiri/Hindi/EN)", "Lowest Platform Commission (10%)"], isPopular: false }
  ];
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">{vendorName || "Hotel"} Dashboard</h1>
            {subscriptionPlan !== "Free" && (
              <span className="bg-gradient-to-r from-orange-200 to-orange-400 text-orange-900 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 uppercase tracking-wider shadow-sm">
                <Award className="w-4 h-4" /> {subscriptionPlan} Vendor
              </span>
            )}
          </div>
          <div className="flex items-center gap-2 mt-1">
            <p className="text-slate-500">Manage your hotel listings, bookings, and billing.</p>
      ...
      <div className="flex gap-6 border-b border-slate-200 mb-8 overflow-x-auto whitespace-nowrap">
        {["overview", "listings", "bookings", "promo"].map((tab) => (
          <button
            key={tab}
            onClick={() => {
              // Block clicking the "listings" tab if they reached the limit AND are not currently editing
              if (tab === "listings" && hasReachedLimit && !editingId) {
                ...
                if (tab === "listings" && !editingId) {
                  handleAddNewClick(); // Reset form if they click Add Property manually
                }
                setActiveTab(tab);
              }
            }}
          ...
          >
            {tab}
            {activeTab === tab && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-orange-500 rounded-t-full" />}
          </button>
        ))}
      </div>
      ...
      {activeTab === "financials" && (
        <div className="space-y-8">
          <div className="bg-orange-500 border border-orange-500 rounded-2xl p-6">
            <h2 className="text-xl font-bold text-orange-500 mb-2">Welcome to WanderKashmir!</h2>
            <p className="text-orange-500">Your registration is complete. Choose a subscription plan below to unlock premium features and increase your bookings.</p>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {subscriptionTiers.map((tier) => {
                ...
                return (
                  <div key={tier.name} className={`bg-white rounded-2xl border relative ${isActive ? 'border-orange-500 shadow-md ring-4 ring-orange-500' : tier.isPopular ? 'border-orange-500 shadow-md' : 'border-slate-200 shadow-sm'} p-6 flex flex-col transition-all`}>
                    {tier.isPopular && !isActive && (
                      <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-orange-500 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">Most Popular</span>
                    )}
                    {isActive && (
                      <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-orange-500 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider flex items-center gap-1">
                ...
                {properties.map((prop: any) => (
                  <div key={prop.id} className="p-6 flex items-center justify-between hover:bg-slate-50 transition-colors">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs font-semibold text-orange-500 mt-1">{prop.availableRooms || 0} / {prop.totalRooms || 0} Rooms Available</p>
          ...
          <div className={`relative bg-white rounded-2xl shadow-sm border border-slate-200 p-6 overflow-hidden mt-8 ${!hasAnalytics ? 'min-h-[350px] bg-slate-50 flex flex-col' : ''}`}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div className="flex items-center gap-2">
                <LineChartIcon className="w-6 h-6 text-orange-500" />
            ...
            {!hasAnalytics && (
              <>
                <div className="absolute inset-0 bg-slate-50/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <button onClick={() => setActiveTab("financials")} className="bg-orange-500 text-white px-6 py-2.5 rounded-lg font-bold hover:bg-orange-500 transition-colors shadow-sm">
                    View Upgrade Plans
                  </button>
                </div>
                ...
                <div className="flex-1 opacity-20 pointer-events-none mt-4 flex items-end gap-2 px-8">
                  <div className="w-1/6 h-24 bg-orange-500 rounded-t-lg"></div>
                  <div className="w-1/6 h-32 bg-orange-500 rounded-t-lg"></div>
                  <div className="w-1/6 h-16 bg-orange-500 rounded-t-lg"></div>
                  <div className="w-1/6 h-40 bg-orange-500 rounded-t-lg"></div>
                  <div className="w-1/6 h-28 bg-orange-500 rounded-t-lg"></div>
                  <div className="w-1/6 h-48 bg-orange-500 rounded-t-lg"></div>
                </div>
              </>
            )}
            {hasAnalytics && (() => {
              ...
              return (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    <button onClick={() => setChartMetric("views")} className={`p-4 rounded-xl border-2 transition-all text-left ${chartMetric === "views" ? 'border-orange-500 bg-orange-500' : 'border-slate-100 bg-white hover:border-slate-200'}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-8 pt-8 border-t border-slate-100">
                    <div className="bg-slate-50 rounded-lg p-4 flex flex-col justify-center border border-slate-100">
                      ...
                      <div className="flex items-end gap-2">
                        <p className="text-xl font-bold text-slate-900">{timeRange === "7D" ? "1,452" : timeRange === "30D" ? "6,210" : "18,450"}</p>
                    ...
                    <div className="bg-slate-50 rounded-lg p-4 flex flex-col justify-center border border-slate-100">
                      ...
                      <div className="flex items-end gap-2">
                        <p className="text-xl font-bold text-slate-900">{timeRange === "7D" ? "89" : timeRange === "30D" ? "310" : "890"}</p>
                        <span className="text-emerald-500 text-xs font-bold mb-1">+5.4%</span>
                      </div>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-4 flex flex-col justify-center border border-slate-100">
                      <p className="text-xs text-slate-500 font-medium mb-1">Booking Conversion</p>
                      <div className="flex items-end gap-2">
                        <p className="text-xl font-bold text-slate-900">3.2%</p>
                        <span className="text-orange-500 text-xs font-bold mb-1">-0.5%</span>
                      </div>
                    </div>
                  </div>
                </>
            ...
            <div className="bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-800 mt-6 text-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white/5 rounded-xl p-4 border border-white/10 flex justify-between items-center">
                  ...
                  <button onClick={() => toast.success("API Keys generated and sent to your email!")} className="bg-orange-500 hover:bg-orange-500 text-white px-4 py-2 text-sm font-bold rounded-lg transition-colors">Generate</button>
                </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-6 border-b border-slate-100 bg-slate-50">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold text-slate-900">Your First Hotel Listing</h2>
          ...
          <div className="p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl space-y-8">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input 
                    type="text" 
                    {...register("name")}
                    className={`w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.name ? 'border-orange-500 focus:border-orange-500' : 'border-slate-200 focus:border-orange-500'}`} 
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className={`w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.description ? 'border-orange-500 focus:border-orange-500' : 'border-slate-200 focus:border-orange-500'}`} 
                ...
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 border-slate-200 focus:border-orange-500`} 
                ...
                <div className="mb-8 border-b border-slate-200 pb-8">
                  ...
                  <ImageUpload 
                    ...
                    setUploadedPhotos={(photos) => {
                      ...
                      // When a new photo is uploaded, it gets appended to the end of the array.
                      // We want to keep the NEW photo, which is the last element.
                  ...
                  {availableAmenities.map(amenity => (
                    <label key={amenity} className="flex items-center gap-3 cursor-pointer">
                      <input 
                        ...
                        className="w-4 h-4 rounded text-orange-500 focus:ring-orange-500 accent-orange-"
                      />
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className={`w-full border rounded-lg pl-10 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 bg-white ${errors.pricePerNight ? 'border-orange-500 focus:border-orange-500' : 'border-slate-200 focus:border-orange-500'}`} 
                        placeholder="4500" 
                      />
                    </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      type="text" 
                      {...register("location")}
                      className={`w-full border rounded-lg pl-10 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.location ? 'border-orange-500 focus:border-orange-500' : 'border-slate-200 focus:border-orange-500'}`} 
                ...
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.totalRooms ? 'border-orange-500 focus:border-orange-500' : 'border-slate-200 focus:border-orange-500'}`} 
                    placeholder="15" 
                  />
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.bedrooms ? 'border-orange-500 focus:border-orange-500' : 'border-slate-200 focus:border-orange-500'}`} 
                    placeholder="1" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.beds ? 'border-orange-500 focus:border-orange-500' : 'border-slate-200 focus:border-orange-500'}`} 
                    placeholder="1" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.guests ? 'border-orange-500 focus:border-orange-500' : 'border-slate-200 focus:border-orange-500'}`} 
                    placeholder="2" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="text" 
                    {...register("bedDetails")}
                    className={`w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 ${errors.bedDetails ? 'border-orange-500 focus:border-orange-500' : 'border-slate-200 focus:border-orange-500'}`} 
              ...
              <div className="bg-orange-500 p-6 rounded-xl border border-orange-500">
                  ...
                  <div className="w-full">
                    ...
                    {editingId ? (
                      <Link 
                        ...
                        className="inline-flex items-center gap-2 bg-orange-500 text-white px-4 py-2 rounded-lg font-bold shadow-sm hover:bg-orange-500 transition-colors"
                      >
              ...
              <div className={`p-6 rounded-xl border ${hasInstantBooking ? 'bg-orange-500 border-orange-500' : 'bg-slate-50 border-slate-200 opacity-70'}`}>
                <div className="flex items-start gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${hasInstantBooking ? 'bg-orange-500 text-white' : 'bg-slate-200 text-slate-400'}`}>
      ...
      {activeTab === "bookings" && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-6 border-b border-slate-100 bg-slate-50">
            ...
            <p className="text-sm text-slate-500 mt-1">Manage all your reservations across your properties.</p>
          </div>
