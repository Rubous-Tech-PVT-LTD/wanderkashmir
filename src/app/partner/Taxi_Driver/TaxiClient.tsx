    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <button 
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-lg font-medium transition-colors shadow-sm ${
            isApproved ? 'bg-orange-500 text-white hover:bg-orange-500' : 'bg-slate-200 text-slate-400 cursor-not-allowed'
          }`}
        >
        ...
        <div className="space-y-8">
          ...
          {(!showVehicleForm && vehicles.length === 0) && (
            <div className="text-center p-12 bg-white rounded-2xl border border-slate-100 shadow-sm">
              ...
              <button 
                onClick={handleAddNewClick}
                className="bg-orange-500 text-white px-6 py-2.5 rounded-lg font-medium hover:bg-orange-500 transition-colors shadow-sm"
              >
          ...
          {showVehicleForm && (
            ...
            <div className="p-6">
              <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl space-y-6">
                ...
                <div className="bg-orange-500 border border-orange-500 rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 mb-2">
                  <div>
                ...
                <div className="flex items-center gap-2">
                  <input type="checkbox" id="ac" {...register("acAvailable")} className="w-4 h-4 text-orange-500 rounded border-slate-300" />
      ...
      {activeTab === "financials" && (
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={plan.name} className={`relative rounded-2xl border-2 p-5 flex flex-col ${subscriptionPlan === plan.name ? 'border-orange-500 bg-orange-500/50' : plan.isPopular ? 'border-indigo-500 bg-indigo-50/10' : 'border-slate-100 bg-white'}`}>
                  ...
                  <ul className="space-y-2 mb-6 flex-1">
                    {plan.features.map(f => (
                      <li key={f} className="flex items-start gap-2 text-xs text-slate-600 font-medium">
                        <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 ${subscriptionPlan === plan.name ? 'text-orange-500' : 'text-slate-400'}`} /> {f}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="bg-orange-500 p-8 rounded-2xl border border-orange-500">
            <h3 className="text-xl font-bold text-orange-500 mb-6 flex items-center gap-2"><Route className="w-6 h-6" /> Dynamic Earnings Calculator</h3>
            <div className="flex flex-col md:flex-row gap-8">
              <div className="flex-1 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-orange-500 mb-2">Total Trip Fare</label>
                  <div className="relative">
                    <IndianRupee className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                    <input type="number" value={calcFare} onChange={(e) => setCalcFare(Number(e.target.value) || 0)} className="w-full border border-orange-500 rounded-lg pl-10 pr-4 py-3 text-lg font-bold text-slate-900 outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500" />
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-2xl font-black text-orange-500">₹{calcFare.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
          </div>
