...
export async function GET() {
  ...
  if (!GOOGLE_ADS_CLIENT_ID || !GOOGLE_REDIRECT_URI) {
    return new NextResponse("Google Ads OAuth configuration missing", { status: 500 });
  }

  // Generate a random state for CSRF protection and append provider prefix
  const state = "ADS_" + crypto.randomBytes(32).toString("hex");
