...
export async function GET(request: Request) {
  ...
  try {
    ...
    const prompt = `
    You are an expert SEO copywriter and Travel Blogger for "WanderKashmir", a premium travel platform in Kashmir.
    Your task is to generate exactly 1 unique, highly-searched SEO landing page or Travel Blog article for Kashmir tourism.
    
    IMPORTANT RULES:
    1. Do NOT use any of these existing routes or blogs: ${existingSlugs.join(", ")}
    2. Pick a very popular, high-traffic topic. It can be a Taxi Route, Homestay Location, or a General Travel Blog (e.g., "Top 10 cafes in Srinagar", "Winter packing list for Kashmir").
    3. The response MUST be a valid JSON object without any markdown wrapping.
    
    JSON STRUCTURE:
    {
      "slug": "the-url-slug-in-kebab-case",
      "type": "TAXI" | "HOMESTAY" | "BLOG",
      ...
      "content": "A detailed 4-5 paragraph SEO optimized content written in Markdown format. If it's a BLOG, make it an engaging article with headings. Mention WanderKashmir.",
      "faqs": [
    ...
    const newPage = await prisma.seoLandingPage.create({
      data: {
        ...
        type: parsedData.type === "HOMESTAY" ? "HOMESTAY" : parsedData.type === "BLOG" ? "BLOG" : "TAXI",
