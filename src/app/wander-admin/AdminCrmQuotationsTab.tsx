...
import { Plus, Search, FileText, CheckCircle, ChevronRight, X, History, Save, Send, Trash, Download } from "lucide-react";
import { format } from "date-fns";

export default function AdminCrmQuotationsTab() {
                ...
                filtered.map((q) => (
                  <tr key={q.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-900">
                      WK-Q-{q.id.substring(0,6).toUpperCase()}
                    </td>
                    <td className="px-6 py-4 text-slate-500">
                      WK-R-{q.requirementId.substring(0,6).toUpperCase()}
                    </td>
                    <td className="px-6 py-4">
                      v{q.version}
                    </td>
                    <td className="px-6 py-4 font-medium text-slate-900">
                      Rs. {q.partnerPrice?.toLocaleString('en-IN') || 0}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        Builder <ChevronRight className="w-4 h-4" />
                      </button>
                    </td>
                  </tr>
                ))
...
function QuotationBuilderModal({ quotationId, onClose, onUpdate }: any) {
        ...
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-900 text-white">
          <div>
            <h3 className="text-xl font-bold flex items-center gap-3">
              Quotation Builder
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-white/20">
                WK-Q-{quotation.id.substring(0,6).toUpperCase()}
              </span>
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-blue-500 text-white">
                v{quotation.version}
              </span>
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-orange-500 text-white">
                {quotation.status}
              </span>
            </h3>
          </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto bg-slate-50 p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              {isEditable && (
                <button className="text-orange-500 hover:text-orange-600 font-medium text-sm flex items-center gap-1 bg-orange-50 px-3 py-1.5 rounded-lg border border-orange-100">
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  {!quotation.items?.length && (
                    <tr>
                      <td colSpan={8} className="px-4 py-8 text-center text-slate-500">No items added to this quotation.</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-2">Inclusions & Exclusions</label>
                    <textarea 
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-0 overflow-hidden flex flex-col">
                 <div className="bg-slate-900 p-4 text-white">
                   <h4 className="font-bold flex items-center justify-between">
                     ...
                     <span className="text-xs bg-white/20 px-2 py-1 rounded text-white/90">INTERNAL</span>
                   </h4>
                 </div>
                 <div className="p-5 space-y-3 flex-1 text-sm">
                   <div className="flex justify-between pb-2 border-b border-slate-100">
                     <span className="text-slate-500">Total Cost</span>
                     <span className="font-medium text-red-600">Rs. {quotation.totalCost}</span>
                   </div>
                   <div className="flex justify-between pb-2 border-b border-slate-100">
                     <span className="text-slate-500">Total Selling (Before Discount)</span>
                     <span className="font-medium">Rs. {quotation.totalSellingPrice}</span>
                   </div>
                   <div className="flex justify-between pb-2 border-b border-slate-100 items-center">
                     ...
                     <div className="flex items-center gap-2">
                       <span className="text-orange-500 font-medium">- Rs. {quotation.discountType === 'FIXED' ? quotation.discount : ((quotation.totalSellingPrice * quotation.discount)/100)}</span>
                     </div>
                   </div>
                   <div className="flex justify-between pb-2 border-b border-slate-100 pt-2">
                     <span className="font-bold text-slate-900">Final Selling Price</span>
                     <span className="font-bold text-green-600 text-lg">Rs. {quotation.partnerPrice}</span>
                   </div>
                   <div className="grid grid-cols-2 gap-4 pt-4 mt-2 bg-slate-50 p-3 rounded-lg border border-slate-100">
                     <div>
                       <span className="block text-xs text-slate-500 mb-1">Gross Margin</span>
                       <span className="font-bold text-slate-700">Rs. {quotation.grossMargin}</span>
                     </div>
                     <div>
                       <span className="block text-xs text-slate-500 mb-1">Net Margin</span>
                       <span className="font-bold text-blue-600">Rs. {quotation.netMargin}</span>
                     </div>
                   </div>
                 </div>
              </div>
            ...
            <div className="p-6 border-b border-slate-100">
              ...
              <div className="space-y-3">
                {quotation.status === "DRAFT" || quotation.status === "REVISED" ? (
                  <button className="w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white py-2.5 rounded-xl font-medium transition-colors">
                    <Send className="w-4 h-4" /> Submit for Approval
                  </button>
                ) : quotation.status === "INTERNAL_REVIEW" ? (
                  <>
                    <button className="w-full flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white py-2.5 rounded-xl font-medium transition-colors">
                      <CheckCircle className="w-4 h-4" /> Approve Quotation
                    </button>
                    <button className="w-full flex items-center justify-center gap-2 bg-red-50 hover:bg-red-100 text-red-600 py-2.5 rounded-xl font-medium transition-colors">
                      Reject Quotation
                    </button>
                  </>
                  ...
                  <>
                     <button className="w-full flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white py-2.5 rounded-xl font-medium transition-colors">
                      <CheckCircle className="w-4 h-4" /> Accept & Convert to Booking
                    </button>
                    <button className="w-full flex items-center justify-center gap-2 bg-orange-50 hover:bg-orange-100 text-orange-600 py-2.5 rounded-xl font-medium transition-colors border border-orange-200">
                      <History className="w-4 h-4" /> Revise (New Version)
                    </button>
                  </>
                ) : null}
              </div>
              ...
              <div className="space-y-4">
                 <div className="p-3 border-2 border-orange-500 rounded-xl bg-orange-50 relative">
                   ...
                   <div className="flex justify-between items-center text-sm">
                     <span className="text-slate-700">Rs. {quotation.partnerPrice}</span>
                     <span className="font-medium text-orange-600 text-xs">{quotation.status}</span>
                   </div>
                 </div>
                 {/* Previous versions would map here */}
                 {quotation.version > 1 && (
                   <div className="p-3 border border-slate-200 rounded-xl bg-slate-50 cursor-pointer hover:bg-slate-100 transition-colors">
                     <div className="font-bold text-slate-600 text-sm">Version {quotation.version - 1}</div>
                     <div className="flex justify-between items-center text-sm mt-1">
                       <span className="font-medium text-slate-500 text-xs">REVISED</span>
                     </div>
                   </div>
                 )}
              </div>
