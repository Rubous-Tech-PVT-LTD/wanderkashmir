...
3. **Failure & Fallback Behavior:**
   - The provider maintains graceful degradation. If Google Ads API calls fail (e.g., rate limits, network failure), the provider returns `UNAVAILABLE`.
   - The GSC (Google Search Console) discovery mechanism remains operational, ensuring the pipeline continues functioning securely without fabricating data.

4. **Net-New Gate & Entity Protection:**
