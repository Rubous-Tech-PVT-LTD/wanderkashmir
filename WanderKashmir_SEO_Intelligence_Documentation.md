...
**Status:** INTEGRATED & MANUAL ENTRY SUPPORTED
- Integrated with Google Ads API v25 (`generateKeywordIdeas`).
- Admin can also directly edit and record manual search volume, competition, and related keywords in the UI.

### Google Trends
**Status:** MANUAL EVIDENCE WORKFLOW
- Official Google Trends deep link provided; admin verifies trend direction, strength, seasonality, and breakout queries manually.

### SERP Intelligence
**Status:** PENDING INTEGRATION (Non-blocking)
- Paid SERP API calls are pending. Strategy generation adapts gracefully using GSC signals, Keyword Planner data, and Trends evidence. Includes one-click live SERP inspection on Google.
...
## SECTION 26 — PHASE 3: LIVE GOOGLE KEYWORD PLANNER INTEGRATION

**Scope:** Replace mock providers with the real Google Keyword Planner API to pull authentic search volume and competition metrics for net-new discovery. Phase 3 focuses solely on keyword discovery and metric evaluation without automating content generation or publishing (which is reserved for Phase 4).

**Dependency Approvals:** Google Ads API Basic Access is now APPROVED.
**Customer ID:** `9633496997`
**Login Customer (Manager) ID:** `6548000449`

### Architecture & Implementation Details

1. **Authentication:** 
   - Reuses the existing Google Ads OAuth flow.
   - Refresh tokens, developer tokens, and client secrets remain strictly server-side and are never exposed to the frontend, browser, or logs.
   
2. **Rate Limiting & Caching:**
   - Implements caching (e.g., Redis or in-memory) to prevent unnecessary repeat API calls during Admin UI refreshes.
   - Supports batched seed processing where possible.
   - Respects Google's documented Keyword Planning API rate limits.
   
3. **Failure & Fallback Behavior:**
- **Net-New Gate & Entity Protection:**
   - All keyword ideas generated by the Keyword Planner are routed through the *existing* Phase 3 Net-New Logic.
   - This prevents entity cannibalization by ensuring that variations matching existing Properties, Tours, Taxis, or Blogs are flagged for human review or rejected, as governed by the golden rules of automation.

---

## SECTION 27 — GOOGLE TRENDS MANUAL EVIDENCE WORKFLOW

### 1. Scope & Objective
The Google Trends Manual Evidence workflow enables Admins to manually verify and record trend signals from the official Google Trends website during the SEO Research phase.
- **Strictly Manual:** No Google Trends scraping, no unauthorized API automations, and no synthetic/estimated trend numbers.
- **Workflow Integrity:** The existing pipeline (`GSC → Opportunity → Research → Strategy → Validation → Admin Approval → Publish`) remains completely intact.
- **Non-Blocking:** Trends evidence is optional; if unprovided, it remains marked as `MANUAL CHECK PENDING`.

### 2. Research Integration Point
1. Admin selects an opportunity or enters a topic in the SEO Research Wizard (`SeoResearchWizard`).
2. Automated research runs for GSC metrics, Cannibalization check, and Keyword Planner.
3. In Step 2 (Research Summary), the **GOOGLE TRENDS — MANUAL CHECK** section displays dynamically generated, topic-specific instructions.
4. Admin opens Google Trends via the provided link (`https://trends.google.com/trends/explore?geo=IN&q=...`), observes trend direction, strength, seasonality, peak/low periods, and breakout queries.
5. Admin inputs the observed evidence into structured fields and clicks "Save Trend Evidence".
6. The evidence is saved and immediately integrated into the active `researchData` context.
7. Subsequent Strategy generation consumes this manual evidence directly into Gemini's strategy prompt.

### 3. Data Source Distinction
Research outputs must strictly differentiate data sources:
- **GSC:** `Real GSC Data` (Clicks, Impressions, CTR, Position).
- **Google Keyword Planner:** `Google Keyword Planner` / `Keyword Planner unavailable`.
- **Cannibalization:** `WanderKashmir DB Inference`.
- **Google Trends:** `Manual Evidence — Google Trends` (Source fixed as `Google Trends — Manual Admin Check`).
*Manually entered Trends observations must NEVER be labeled as GSC data, Keyword Planner data, API-fetched Trends, or estimated search volume.*

### 4. Topic-Specific Instruction Generation
- **Primary Keyword:** Exact opportunity topic or primary query.
- **Comparison Keywords:** Pulled from existing GSC top/related queries or GKP related queries. If unavailable, clearly displays `"No verified comparison keyword available."` No unverified comparisons are fabricated.
- **Recommended Settings:**
  - Country: `India` (`geo=IN`)
  - Search Type: `Web Search`
  - Time Range: `Past 12 months`
  - Category: `Travel` (when applicable)
- **Checklist:** Interest over time, trend direction (rising/stable/declining), strength (strong/moderate/weak), seasonality (yes/no), peak period, lowest period, related rising queries/topics.

### 5. Persistence Architecture (Zero Schema Migrations)
- **Database Safety:** Safe persistence without schema alterations or migrations.
- **Storage Strategy:**
  - Full structured evidence is persisted in `SystemConfig` under key `seo_trend_evidence:${canonicalTopic}`.
  - If a matching `SeoOpportunity` exists, `SeoOpportunity.googleTrends` is updated with a human-readable summary (e.g., `MANUAL: RISING (STRONG)`).
  - When researching or publishing, the structured `ManualTrendEvidence` is saved directly inside `SeoLandingPage.seoResearch` JSON.
- **Destructive Operations:** Absolutely zero `DROP`, `DELETE`, `TRUNCATE`, or table rewrites.

### 6. Validation Rules & Safety Constraints
- Server-side validation via `POST /api/admin/seo-intelligence/trends-evidence`.
- Enforces valid enum states:
  - `trendDirection`: `RISING` | `STABLE` | `DECLINING` | `UNCLEAR`
  - `trendStrength`: `STRONG` | `MODERATE` | `WEAK` | `UNCLEAR`
  - `seasonality`: `YES` | `NO` | `UNCLEAR`
- All text inputs (peak period, lowest period, queries, topics, notes) are sanitized and escaped to prevent XSS / injection attacks (no `dangerouslySetInnerHTML`).
- Source string is immutable: `"Google Trends — Manual Admin Check"`.

### 7. Affected Files
- `WanderKashmir_SEO_Intelligence_Documentation.md`: Architecture and workflow documentation.
- `src/lib/seo/types.ts`: `ManualTrendEvidence` interface and `SeoResearch` updates.
- `src/lib/seo/research-engine.ts`: Check existing saved manual trend evidence for research target.
- `src/app/api/admin/seo-intelligence/research/route.ts`: Pass `opportunityId` to `runSeoResearch`.
- `src/app/api/admin/seo-intelligence/trends-evidence/route.ts`: API endpoint to validate and save evidence.
- `src/lib/seo/strategy-engine.ts`: Ingest manual evidence into Gemini prompt, explicitly prevent assumptions when pending.
- `src/app/wander-admin/AdminSeoTab.tsx`: Dynamic manual trends instruction card, input form, and status pills in `SeoResearchWizard`.

### 8. Acceptance Criteria
- [x] Admin can launch Research on any GSC opportunity or custom topic.
- [x] Dynamic topic-specific instructions and comparison keywords are presented.
- [x] Link to official Google Trends opens with proper geo/keyword parameters.
- [x] Admin can enter and save structured trend evidence with immediate UI feedback.
- [x] Saved evidence persists and reloads upon re-researching the topic.
- [x] Strategy engine incorporates manual evidence or marks Trends as pending without making false assumptions.
- [x] Existing GSC, GKP, cannibalization, and publishing workflows are completely intact.
